...
<section id="education" class="w-full fade-in-delay-3">
	<h2 class="text-xl font-semibold mb-2 text-gray-50 leading-snug px-4 pt-6">Education</h2>
		...
		{#if education.degree}
			<article class="py-5 px-4 first:pt-6">
				<div class="flex items-center gap-3 mb-3">
					...
					<h2 class="text-xl font-semibold text-gray-50 leading-snug">
						{education.degree}
						{#if education.specialization}
				...
				<p class="text-base leading-relaxed text-gray-400 mb-3 whitespace-pre-line">
					{education.institution}
				</p>
				{#if education.period}
					<div class="text-sm text-gray-400 mt-2">{education.period}</div>
				{/if}
			</article>
		{/if}
	{/each}
